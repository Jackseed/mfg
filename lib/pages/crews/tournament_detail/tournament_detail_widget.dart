...
class _TournamentDetailWidgetState extends State<TournamentDetailWidget> {
  ...
  /// Load matchups + decks across all user's crews.
  Future<_TournamentData> _loadTournamentData(TournamentsRecord tournament) async {
    // 1. Get matchups for this tournament
    final matchups = await queryMatchupsRecordOnce(
      queryBuilder: (q) =>
    ...
    // 2. Multi-crew deck lookup: get user's crewmate, find spicerack userId,
    //    then all crewmates across crews, then all crewIds → query decks
    final myCrewmateRef = currentUserDocument?.crewmateRef;
    Set<String> myDeckIds = {};
    List<DecksRecord> allDecks = [];

    if (myCrewmateRef != null) {
      // Get main crewmate doc to find spicerack userId
      final myCrewmateDoc = await myCrewmateRef.get();
      final spicerackUserId = myCrewmateDoc.data() != null
          ? (myCrewmateDoc.data() as Map<String, dynamic>)['userId'] as String?
          : null;

      if (spicerackUserId != null && spicerackUserId.isNotEmpty) {
        // Find ALL crewmates across all crews with same spicerack userId
        final allCrewmates = await FirebaseFirestore.instance
            .collectionGroup('crewmates')
            .where('userId', isEqualTo: spicerackUserId)
            .get();
        ...
        for (final doc in allCrewmates.docs) {
          ...
          // Parent path: crews/{crewId}/crewmates → parent.parent = crews/{crewId}
          final crewDocRef = doc.reference.parent.parent;
          if (crewDocRef != null) {
            allCrewIds.add(crewDocRef.id);
          }
        }

        // Query decks across all crews (up to 30 per whereIn)
        if (allCrewIds.isNotEmpty) {
          final crewIdList = allCrewIds.toList();
          for (var i = 0; i < crewIdList.length; i += 30) {
            final batch = crewIdList.sublist(
                i, i + 30 > crewIdList.length ? crewIdList.length : i + 30);
            final batchDecks = await queryDecksRecordOnce(
              queryBuilder: (q) => q.where('crewId', whereIn: batch),
            );
            allDecks.addAll(batchDecks);
          }
        }

        // Build myDeckIds from all crewmate IDs
    ...
    // Fallback: if no decks found via multi-crew, try single crew
    if (allDecks.isEmpty) {
      final crewId = valueOrDefault(currentUserDocument?.crewId, '');
      if (crewId.isNotEmpty) {
        allDecks = await queryDecksRecordOnce(
          queryBuilder: (q) => q.where('crewId', isEqualTo: crewId),
        );
        final myCrewmateId = myCrewmateRef?.id ?? '';
        for (final d in allDecks) {
          if (d.crewmateId == myCrewmateId && d.deckId.isNotEmpty) {
            myDeckIds.add(d.deckId);
          }
        }
      }
    ...
    // Build crewmate name map (crewmateId → player name)
    final crewmateRefs = allDecks
